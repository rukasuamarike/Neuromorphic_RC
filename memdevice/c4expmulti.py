...
class BiolekC4Memcapacitor(nn.Module):
    ...
    def UpdateMultiRho(self, V, dt):
        ...
        # Broadcast Cmin and Cmax to match the shape of Rho
        minVector = self.Cmin.expand_as(self.Rho)
        maxVector = self.Cmax.expand_as(self.Rho)

        # Use torch.clamp for element-wise clipping
        self.Rho = torch.clamp(self.Rho, min=minVector, max=maxVector)
        # calculating the charge
        self.Q = torch.mul(self.Rho, V)
...
if __name__ == "__main__":
    ...
    Vs2 = Ampl * np.sin(np.pi * 2 * Freq * ts)
    ...
    for v,v2 in zip(Vs,Vs2):
        vv = torch.tensor([v,v2], dtype=torch.float64)
        out = MemcapDevice.ComputeCap(vv, dt)
        DictVals["QCal"].append(out[0].tolist())
        DictVals["MemC"].append(out[1].tolist())
        DictVals["i"].append(out[2].tolist())
